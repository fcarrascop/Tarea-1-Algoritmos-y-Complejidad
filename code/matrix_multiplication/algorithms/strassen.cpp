...
namespace {
    ...
    // Modificacion respecto a la fuente: version entera (bit shift) en lugar de pow(2, ceil(log2(n))), para evitar errores de redondeo de punto flotante.
    ...
    vector<vector<int>> resizeMatrix(const vector<vector<int>> &mat,
                                            int newR, int newC) {
...
// Multiply mat1 (n x m) and mat2 (m x q) using Strassen's method.
vector<vector<int>> strassen_multiply(const vector<vector<int>> &mat1,
    ...
    // Extract the valid n x q result from the padded result
